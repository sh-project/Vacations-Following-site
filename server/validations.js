...
const checkImage = (file) => {
    ...
    if (file.size > 300000) return 'The file is too large.';
}

const checkFileName = (fileName) => {
    if (!fileName) return 'not file'

    const fileNameRegex = /^[0-9a-zA-Z\^\&\'\@\{\}\[\]\,\$\=\!\-\#\(\)\.\%\+\~\_ ]+$/;
    if (!(fileNameRegex.test(fileName))) {
        return 'File name is forbidden.'
    }
}
...
module.exports = { checkUsername, checkName, checkImage, checkFileName, checkPassword, checkConfirmPassword, checkRequiredDate, checkFreeText, checkOptionalFreeText, checkNumber };